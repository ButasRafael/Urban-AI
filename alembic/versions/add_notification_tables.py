...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('delivered_channels', postgresql.JSON(astext_type=sa.Text()), nullable=True),
    ...
    op.create_index('ix_notification_log_username_read', 'notification_log', ['username', 'read_at'])
...
def downgrade():
    ...
    op.drop_index('ix_notification_log_username_read', table_name='notification_log')
    ...
    # Drop enum types using postgresql.ENUM to match upgrade()
    ...
    postgresql.ENUM(name='event_type_enum').drop(op.get_bind(), checkfirst=True)
